...
export default function AdministradorCotizaciones() {
  ...
  const cargarCotizaciones = async () => {
    try {
      setLoading(true);
      const response = await fetch(`${API_URL}/api/sales/quotes/`);
      if (response.ok) {
        const data = await response.json();
        const cotizacionesFormateadas = data.results?.map((q: any) => ({
          id: q.id,
          numero_cotizacion: q.quote_number || `COTIZ-${q.id}`,
          empresa: q.opportunity?.lead?.company_name || 'N/A',
          email: q.opportunity?.lead?.email || 'N/A',
          estado: q.status || 'pendiente',
          monto: q.total_amount || 0,
          fecha_creacion: new Date(q.created_at).toLocaleDateString('es-EC'),
          tipo: 'automatica'
        })) || [];
      ...
      <div className="max-w-7xl mx-auto px-4">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600">
            Consulta, administra y asigna tus cotizaciones a Routing Orders
          </p>
        </div>
                    ...
                    <tr key={cot.id} className="border-b border-gray-200 hover:bg-slate-50 transition-colors">
                      ...
                      <td className="px-6 py-4 text-sm">
                        <span className={`px-3 py-1 rounded-full text-xs font-medium ${
                          cot.estado === 'enviado' ? 'bg-blue-100 text-blue-800' :
                      ...
                      <td className="px-6 py-4 text-sm">
                        <div className="flex items-center gap-2">
                          ...
                          <button
                            className="p-2 hover:bg-velocity-green/10 rounded-lg transition-colors"
                            title="Copiar número"
                          >
