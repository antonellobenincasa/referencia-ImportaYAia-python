...
import { FileText, Search, Filter, Eye, Copy, ExternalLink } from 'lucide-react';
      ...
      <div className="max-w-7xl mx-auto px-4">
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead className="bg-slate-100 border-b border-gray-200">
                  <tr>
                    <th className="px-6 py-3 text-left text-sm font-semibold text-gray-900">Nº Cotización</th>
                ...
                <tbody>
                  {cotizacionesFiltradas.map((cot) => (
                    <tr key={cot.id} className="border-b border-gray-200 hover:bg-slate-50 transition-colors">
                      <td className="px-6 py-4 text-sm font-mono font-semibold text-velocity-green">{cot.numero_cotizacion}</td>
                      <td className="px-6 py-4 text-sm text-gray-900">{cot.empresa}</td>
                      <td className="px-6 py-4 text-sm font-semibold text-gray-900">${cot.monto.toFixed(2)}</td>
        ...
        <div className="mt-6 grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="bg-gradient-to-br from-velocity-green/10 to-velocity-green/20 rounded-lg p-4 border border-velocity-green/30">
            <p className="text-sm font-semibold text-green-900">Automáticas</p>
            <p className="text-2xl font-bold text-velocity-green mt-1">{cotizaciones.filter(c => c.tipo === 'automatica').length}</p>
          </div>
          <div className="bg-gradient-to-br from-aqua-flow/10 to-aqua-flow/20 rounded-lg p-4 border border-aqua-flow/30">
            <p className="text-sm font-semibold text-teal-900">Manuales</p>
            <p className="text-2xl font-bold text-aqua-flow mt-1">{cotizaciones.filter(c => c.tipo === 'manual').length}</p>
          </div>
        </div>
      </div>
    </div>
